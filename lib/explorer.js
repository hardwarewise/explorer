...
module.exports = {
  ...
  get_gettermdepositstats: function(cb) {
    var uri = 'http://127.0.0.1:' + settings.port + '/gettermdepositstats';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },

  get_officialpoolinfo: function(cb) {
    var uri = 'http://127.0.0.1:' + settings.port + '/officialpoolinfo';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },

  get_infinitynode: function(cb) {
    var uri = base_url + 'infinitynode?param=show-infos';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },
  ...
  exec_blockhash: function(height, cb) {
    var cmd = settings.wallet.cli + ' getblockhash ' + height;
    exec(cmd, function (error, stdout, stderr) {
     if (error) {
       console.log('exec_blockhash error: ' + error);
       return;
     }
     if (stderr) {
       console.log('exec_blockhash stderr: ' + stderr);
       return;
     }
     return cb(stdout);
    });
  ...
  get_blockhash: function(height, cb) {
    ...
    request({uri: uri, json: true, timeout: 4000}, function (error, response, body) {
      return cb(body);
    });
  },

  exec_block: function(hash, cb) {
    var cmd = settings.wallet.cli + ' getblock ' + hash;
    exec(cmd, function (error, stdout, stderr) {
     if (error) {
       console.log('exec_block error: ' + error);
       return;
     }
     if (stderr) {
       console.log('exec_block stderr: ' + stderr);
       return;
     }
  ...
  exec_rawtransaction: function(hash, cb) {
    var cmd = settings.wallet.cli + ' getrawtransaction ' + hash + ' true';
    exec(cmd, {maxBuffer: 1024 * 1024 * 4}, function (error, stdout, stderr) {
     if (error) {
       console.log('exec_getrawtransaction error: ' + error);
       return;
     }
     if (stderr) {
       console.log('exec_getrawtransaction stderr: ' + stderr);
       return;
     }
     return cb(JSON.parse(stdout));
    });
  ...
  stats_active_address: function(cb) {
    Address.find({}, 'balance').where('balance').gt(1000000000).exec(function(err, docs) {
      return cb(docs.length);
    });
  },
  ...
  get_supply: function(cb) {
    ...
    } else if (settings.supply == 'SUMMARY') {
      var uri = 'http://127.0.0.1:' + settings.port + '/summary';
      request({uri: uri, json: true}, function (error, response, body) {
        return cb(body.supply);
      });
    } else {
